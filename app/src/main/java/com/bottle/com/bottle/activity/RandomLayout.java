...
public class RandomLayout extends ViewGroup {
	...
	public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        HashMap<View, HashMap<String, Integer>> HashMap = new HashMap<View, HashMap<String, Integer>>();
        for (int i = 0; i < count; i++) {
            ...
            if (!mFixedViews.contains(child)) {// mFixedViews用于存放已经确定好位置的View，存到了就没必要再次存放
                ...
                // 先测量子View的大小
                int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(this.getMeasuredWidth(), MeasureSpec.UNSPECIFIED);// 为子View准备测量的参数
                int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(this.getMeasuredHeight(), MeasureSpec.UNSPECIFIED);
                ...
                while (availAreaCount > 0) { // 如果使用区域大于0，就可以为子View尝试分配
                    ...
                    if (mAreaDensity[row][col] < areaCapacity) {// 区域密度未超过限定，将view置入该区域
                        ...
                        if (!isOverlap(params)) {// 判断是否和别的View重叠了
                            ...
                            HashMap<String, Integer> childHashMap = new HashMap<String, Integer>();
                            childHashMap.put("mLeft", params.mLeft);
                            childHashMap.put("mTop", params.mTop);
                            childHashMap.put("mRight", params.mRight);
                            childHashMap.put("mBottom", params.mBottom);
							/*if(i > 5 ){
								ImageView bottle_h =  (ImageView)  child.findViewById(R.id.bottle_h);
								FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams)bottle_h.getLayoutParams();
								FrameLayout.LayoutParams params1 = new FrameLayout.LayoutParams(130,130);
								bottle_h.setLayoutParams(params1);
							}*/

                            HashMap.put(child, childHashMap);
                            //child.measure(childWidthMeasureSpec,childWidthMeasureSpec);
                            //child.layout(childHashMap.get("mLeft"), childHashMap.get("mTop"), childHashMap.get("mRight"), childHashMap.get("mBottom"));// 布局子View
	...
	if(null != HashMap && HashMap.size() > 0){
		int i = 0;
		java.util.HashMap<Integer, View> h = new HashMap<Integer, View>();
		List<Integer> list = new ArrayList<Integer>();
		for (View view : mFixedViews) {
			java.util.HashMap<String, Integer> viewHashMap = HashMap.get(view);
			h.put(viewHashMap.get("mTop")+i, view);

			list.add(viewHashMap.get("mTop")+i);i++;
		}
		...
		for (int j = 0; j < list.size(); j++) {
			{
				final View view = h.get(list.get(j));
				final java.util.HashMap<String, Integer> viewHashMap = HashMap.get(view);
				if (null != viewHashMap) {

					ImageView bottle_h = (ImageView) view.findViewById(R.id.bottle_h);
					LinearLayout.LayoutParams params1 = (LinearLayout.LayoutParams) bottle_h.getLayoutParams();
					//LinearLayout.LayoutParams params1 = new LinearLayout.LayoutParams(layoutParams);
					int childH_height = (int) (childH-((childH*0.6)*((j+1) * 0.1)));
					int childH_width =  (int) (childW-((childW *0.6 )*((j+1) * 0.1)));
					params1.height= childH_height < ((int)(childH*0.4)) ? (int) (childH * 0.4) : childH_height;//;
					params1.width= childH_width < ((int)(childW*0.4)) ? (int) (childW *0.4) : childH_width;//50+(j *10);
					params1.gravity= Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL;
					bottle_h.setLayoutParams(params1);
					int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(this.getMeasuredWidth(), MeasureSpec.UNSPECIFIED);// 为子View准备测量的参数
					int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(this.getMeasuredHeight(), MeasureSpec.UNSPECIFIED);
					view.measure(childWidthMeasureSpec, childHeightMeasureSpec);
					view.layout(viewHashMap.get("mLeft"), viewHashMap.get("mTop"), viewHashMap.get("mRight"), viewHashMap.get("mBottom"));// 布局子View
