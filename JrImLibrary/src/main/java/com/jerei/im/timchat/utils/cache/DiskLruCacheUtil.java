package com.jerei.im.timchat.utils.cache;
...
public class DiskLruCacheUtil {
	...
	public static DiskLruCache open(Context context){
		try {//  data/data/com.android.httpclient10_21/cache/bitmap
			 ...
			 */
			mDiskLruCache=DiskLruCache.open(cacheDir, getAppVersion(context), 1, 1024*1024*1024);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * 从缓存中读取**********************************************外部调用
	 */
	public static InputStream readFromDiskCache(String url, Context context){
		try {
			open(context);
			//得到key
			String key=hashKeyForDisk(url);
