...
public class GroupMemberProfileActivity extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(TextUtils.isEmpty(profile.getNameCard())){
            userCard = profile.getName();
        }else {
            userCard = profile.getNameCard();
        }


        roundCornerImageView = (RoundCornerImageView) findViewById(R.id.headimage);
        GetHeadImageUtil.setImageUrl(roundCornerImageView,profile.getAvatarUrl(),this);
...
//    }
    private boolean canManage(){
        if ((currentUserRole == TIMGroupMemberRoleType.Owner &&!UserInfo.getInstance().getId().equals(userIdentify)))
        {
            return true;
        }
        return false;
